...
def test_send_parent_message_email_logs_when_smtp_not_configured(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
    monkeypatch.setattr(settings, "smtp_host", None)

    with caplog.at_level(logging.INFO, logger="app.email"):
        email.send_parent_message_email("veli@example.com", "Konu", "Mesaj metni")

    assert "SMTP not configured" in caplog.text
    assert "veli@example.com" in caplog.text


def test_send_parent_message_email_sends_via_smtp(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings, "smtp_host", "smtp.example.com")
    monkeypatch.setattr(settings, "smtp_port", 587)
    monkeypatch.setattr(settings, "smtp_username", "user@example.com")
    monkeypatch.setattr(settings, "smtp_password", "secret")
    ...
    mock_smtp_instance.__enter__.return_value = mock_smtp_instance
    with patch("smtplib.SMTP", return_value=mock_smtp_instance) as mock_smtp:
        email.send_parent_message_email("veli@example.com", "Karne hakkında", "Ada bu hafta çok iyi ilerledi.")
    ...
    mock_smtp_instance.send_message.assert_called_once()
    sent_message = mock_smtp_instance.send_message.call_args[0][0]
    assert sent_message["To"] == "veli@example.com"
    assert sent_message["From"] == "no-reply@example.com"
    assert sent_message["Subject"] == "Karne hakkında"
    assert "Ada bu hafta çok iyi ilerledi." in sent_message.get_content()


def test_send_parent_message_email_swallows_smtp_errors(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
    monkeypatch.setattr(settings, "smtp_host", "smtp.example.com")
    ...
    with patch("smtplib.SMTP", side_effect=smtplib.SMTPConnectError(421, "unreachable")):
        with caplog.at_level(logging.ERROR, logger="app.email"):
            email.send_parent_message_email("veli@example.com", "Konu", "Mesaj")

    assert "Failed to send parent message email" in caplog.text
